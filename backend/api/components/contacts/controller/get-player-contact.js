...
async function getPlayerContact(req, res, next) {
  try {
    ...
    const getContactsByPlayerId = `select  a.*, d.* from (SELECT contacts.contactId as contactId,contacts.contactPlayerId as playerId, contacts.contactScoutId as scoutId, contacts.contactDescription as message, contacts.contactStatus as contacSstatus, users.userImage as userImage, users.userName as userName, users.userNumber as userNumber FROM contacts  inner join users  on contacts.contactPlayerId = users.userId) a  inner join 
(SELECT contacts.contactId as contactId, users.userImage as scoutImage, users.userName as scoutName, contacts.contactPlayerId as playerId FROM contacts   inner join users  on contacts.contactScoutId = users.userId) d on (a.playerId = d.playerId) and a.contactId = d.contactId and a.playerId = ?
