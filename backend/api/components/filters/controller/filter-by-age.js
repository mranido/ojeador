...
async function getCategory(req, res, next) {
  ...
  try {
    ...
    const getAge = `SELECT userId, userName, timestampdiff(year, userbirthday, curdate()) as userAge, userPosition, userTeam FROM users where userRol ='Player';`;
    ...
    const category = dataUserWithCategory.filter(
      (item) => findCategory === item.category
    );
